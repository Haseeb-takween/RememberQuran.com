...
| 4 | **Quran.com audio CDN** (`audio.qurancdn.com`, `download.quranicaudio.com`) | **Word-by-word pronunciation files** (`/wbw/001_001_001.mp3` — `Word.audio_url` is relative to `audio.qurancdn.com`); full-chapter recitation MP3s served from `download.quranicaudio.com` | M2 | **Verified 2026-07 — primary (M2 shipped).** See `docs/audio-sources.md` |
| 5 | **EveryAyah.com** | Per-ayah audio for many reciters; word/ayah timing data | M5 | **Not needed for M2** — QDC segments (#6) cover both launch reciters. Re-evaluate per reciter at M5 scale-up |
| 6 | **QDC audio endpoint with segments** (`api.qurancdn.com/api/qdc/audio/reciters/{id}/audio_files?chapter={n}&segments=true`) | One MP3 per surah + `verse_timings` with word-level segments `[wordPosition, startMs, endMs]` for word-highlight sync | M2 | **Verified 2026-07 — primary (M2 shipped).** Alafasy (7) and Sudais (3) both have segments. See `docs/audio-sources.md` for data-quality gotchas |
